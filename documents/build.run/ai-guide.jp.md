...
AI が検索に使う前提知識は 2 段階に分かれています

| | 置き場所 | 効く範囲 | 誰が書くか |
|---|---|---|---|
| MCP 側のノート | MCP サーバーが読む Markdown ファイル | アプリ内チャットとすべてのエージェント | 管理者 |
| エージェント側のノート | エージェントが読む指示ファイル（Claude Code では Skill） | そのエージェントだけ | 利用者 |

ツールの使い分けや日付の解釈といった一般的な前提はサーバーに同梱されているので、どちらも書かなくても動きます

### MCP 側のノート

タグやフォルダの意味、担当など、チーム固有の前提を書きます
...
エージェントに「動画の質問では video-review MCP を使い、前提知識を読んでから答える」と指示するファイルです  
リポジトリの `.claude/skills/video-review-search/SKILL.md` にその内容が入っています

Claude Code では、リポジトリで開くと自動で読まれます  
別のディレクトリから使う場合は `~/.claude/skills/` にコピーします

```bash
cp -r .claude/skills/video-review-search ~/.claude/skills/
```

Codex CLI では `AGENTS.md`、Gemini CLI では `GEMINI.md` が同じ役割の指示ファイルです  
`SKILL.md` の本文を、それぞれのファイルに写します

自分のエージェントだけに効かせたい指示（回答の形式、よく聞く質問の言い回しなど）は、この指示ファイルに追記します  
チーム全員に効かせたい前提は MCP 側のノートに書きます
