...
// ---------------------------------------------------------------------------
// Integration tests — only run when GitHub token is configured
// ---------------------------------------------------------------------------

const integrationOwner = process.env.VIDEO_REVIEW_VCS_GITHUB_OWNER;
const integrationRepo  = process.env.VIDEO_REVIEW_VCS_GITHUB_REPO;
const integrationToken = process.env.VIDEO_REVIEW_VCS_GITHUB_TOKEN;
const hasCredentials   = !!(integrationOwner && integrationRepo && integrationToken);

describe.skipIf(!hasCredentials)("GitHubProvider (integration — real API)", () => {
    it("fetches real changes from the configured repository", async () => {
        const realProvider = new GitHubProvider({
            owner: integrationOwner!,
            repo:  integrationRepo!,
            token: integrationToken!,
            branch: process.env.VIDEO_REVIEW_VCS_BRANCH ?? "main",
        });

        const to   = new Date();
        const from = new Date(to.getTime() - 90 * 24 * 60 * 60 * 1000); // 90 days ago

        const result = await realProvider.getChanges({ from, to });
        ...
        // Spot-check shapes if results exist
